...
class hopfield_network:
    def __init__(self,N):
        ...
        INPUT
        N: size of the network, i.e. if N=10 it will consist of 10 pixels

        -L.Ziegler 03.2009.
        """

    def make_pattern(self,P=1,ratio=0.5,E = 1):
        """
        DEFINITION
        ...
        # Create a network with an excitatory and inhibitory population depending on E
        self.network = -ones(self.N)
        idx2 = int(E*self.N)
        self.network[:idx2] = 1
        self.network = permutation(self.network)
        ...
        for i in range(self.N):
            for j in range(self.N):
                if i != j:
                    ...
                    # If weight[i,j] and network[i] are not of the same sign, weight[i,j] = 0
                    if  (self.network[i]*self.weight[i,j]) < 0:
                        self.weight[i,j] = 0

    def create_pattern(self,ratio=0.5):
        """
        DEFINITION
        creates a new pattern and adds it at the end of the self.pattern matrix
